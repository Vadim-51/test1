...
wsServer.on('request', function(request) {
  ...
  var is_in = '';
  //check if user participates in a video chat
  /*var usV
  function isInChat(name){
	 usV = false;
   	 for (var i = 0; i < videoChats.length; i++) {
    if (videoChats[i].name == name) {
        usV = true;
    }
   }
   return usV;
 ...
 var broadcasters = {};
      switch(msg.type) {
        ...
        case "username":
		 ...
		 /* for(var t in testBroadcasters){
			  ...
			  var arr = testBroadcasters[t];
			  arr.relayReceivers = relayReceivers;
		  }*/
    ...
    testBroadcasters[msg.name] = {
		  ...
		  var array = [];
	      var msgChat = {
		  type: 'videochat-number',
		  array: testBroadcasters, //Send information about videochat participants
		  target: msg.target,
		  check: is_in,
		  lastP: last_added_person,
		  upList: upd_relayReceivers
	       }
		   var msgStringV = JSON.stringify(msgChat);
	       for (i=0; i<connectionArray.length; i++) {
           connectionArray[i].sendUTF(msgStringV);
