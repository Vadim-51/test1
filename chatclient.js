...
var broadcasters = {};
  ...
  connection.onmessage = function(evt) {
    ...
    var text = ""; //defining a variable in wich wi'll put responses from the server, preliminary correcting them
    ...
    switch(msg.type) {
	  ...
	  case "videochat-number":
	      var lastP = msg.lastP;
		  var upList = msg.upList;
		  console.log('Last Person..............' + lastP);
		  console.log('Updated List................' + upList);
		  targetNam = msg.target;
		  broadcasters = msg.array;
		  handleWebchats(broadcasters);
		  is_in = msg.check;
		  console.log('********** is_in is: '+msg.check);
		  console.log(broadcasters);
		  console.log('Target answer is ' + msg.target);
		   var an = 'andrey';
		   var va = 'vad';
		   if(broadcasters[an]){
			document.getElementById("chats").innerHTML += 'There is andrey in broadcasters ' 
			+broadcasters.length+'<br>';
			
		   }else{
			document.getElementById("chats").innerHTML += 'No, there is no andrey in broadcasters '+
			broadcasters.length+'<br>';
		   }
		   if(broadcasters[va]){
			document.getElementById("chats").innerHTML += 'There is vad in broadcasters ' 
			+broadcasters.length+'<br>';
			
		   }else{
			document.getElementById("chats").innerHTML += 'No, there is no vad in broadcasters '+
			broadcasters.length+'<br>';
		   }
		   document.getElementById("chats").innerHTML += 'The length of testBroadcaster is '+ broadcasters.length+'<br>'; 
		   console.log('testBroadcaster is:::::::::::::::::::::::: ' + broadcasters);
		  break;
...
function handleWebchats(users){

		
   for(var n in users) {
	    document.getElementById("chats").innerHTML += 'Is_in is : ' + is_in+'<br>'
	    document.getElementById("chats").innerHTML += 'Target: ' + targetNam+'<br>';
		document.getElementById("chats").innerHTML += users[n] + '<br>';
        document.getElementById("chats").innerHTML += users[n].name + '<br>';  
        document.getElementById("chats").innerHTML += 'Broadcast Id ' + users[n].broadcastId + '<br>';
        document.getElementById("chats").innerHTML += 'relayReceivers ' +users[n].relayReceivers + '<br>';
        document.getElementById("chats").innerHTML += 'receivingFrom ' +users[n].receivingFrom + '<br>';
        document.getElementById("chats").innerHTML += 'isBroadcastInitiator ' +users[n].isBroadcastInitiator + '<br>';
        console.log('/////////////////Relay receivers: '+users[n].relayReceivers);
      /*for(var i in users[n]){
		  document.getElementById('chats').innerHTML += users[n][i]+ '<br/>';
	  }*//*
	  for(var t in users){
      var arr = users[t];
      document.getElementById('chats').innerHTML += arr.name+'<br>';
}*/
...
function handleVideoAnswerMsg(msg) {
  ...
  var desc = new RTCSessionDescription(msg.sdp);
  myPeerConnection.setRemoteDescription(desc).catch(reportError);//mayby at this place it's possible to create an array of users that have the same stream
}
